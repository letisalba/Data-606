...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    * **_A streak length of 1 means that there was only 1 hit in the series. In a streak length of 0 means there were 0 hits in the series._**
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    * **_Based on the chart above, 0 and 1 have the highest count in streaks and as the streak length gets higher the counts decrease. The longest streak is 4 baskets._**
    
    
```{r}
#Summary of kobe_streak
summary(kobe_streak)
```


```{r}
# Bar chart streaks done by Kobe
ggplot(data = kobe_streak, aes(x = length)) +
  geom_bar()
```
...
```{r set-seed}
set.seed(26873)                 # make sure to change the seed
```
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    * **_See code below to see the changes made to the funtion so that it reflects 45% of shooting percentage._** 
```{r}
#Set seed
set.seed(26873)
...
```{r}
# Computing streak lengths using 'calc_streak'
...
6.  Describe the distribution of streak lengths. What is the typical streak 
    ...
    * **_The longest streak is 4 according to my plot._**
...
7.  If you were to run the simulation of the independent shooter a second time, 
    ...
    * **_Being that the simulation of the independent shooter is at a 45% the results for the second time would be very similar as the first. If there is any slight variation it could be seen in the longest streak (3 or 4) or there could be a very small longest streak added._**
...
8.  How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    * **_Based on the bar charts below, both streak distributions are right skewed, have a higher count in 0 and 1 and their longest length is 4. One noticeable difference is that with Kobe, the length of 2 and 3 have a similar streak count as opposed to the Simulated chart where length 2 is of much higher count than 3. This is not evidence that the hot hand model fits Kobe's shooting patterns as it appears to be independent from each other._**
...
```{r}
#Bar chart for simulated shooter
ggplot(data = sim_streak, aes(x = length)) +
  geom_bar()
```
